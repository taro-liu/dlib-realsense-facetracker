    ...
    try:
        while True:
            ...
            for face in faces:
                ...
                tmp = 1
                for n in [left_eyebrow, right_eyebrow, left_eye, right_eye, left_ear, right_ear, nose, mouse]: #
                    x, y = xy_mean(n)
                    ...
                    text = str([a, b, c])
                    cv2.putText(img, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), thickness=1)
                    cv2.circle(img, (x, y), 1, (0, 255, 0), thickness=1)
                    tmp += 1
                    if tmp == 6:
                        tmp_text = text
                    if tmp == 7:
                        tmp_text = str(tmp_text+'    '+text)
                    if tmp == 8:
                        tmp_text = str(tmp_text+'    '+text)
                        file.file(tmp_text+"\n") # 耳朵的真实三维坐标存储到txt文件
